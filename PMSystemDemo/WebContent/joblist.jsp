	...
	<div id="content">
		...
		<div id="breadcrumb">
			<a href="main.jsp" class="tip-bottom"> <i class="icon-home"></i>
				首页
			</a> <a href="${pageContext.request.contextPath }/GetAllJobServlet"
				class="current">职位信息管理</a>
		</div>
		<div class="container-fluid">
			<div class="row-fluid">
				<div class="control-group">
					<span class="span3"> 编号：<input id="btnId" name="id"
						type="number" style="width: 65%" />
					</span> <span class="span3">
						<button class="btn btn-info" onclick="selectById()">
							<i class="icon-white icon-search"></i> 查询
						</button> <input type="reset" name="button" id="button" value="重置" />
					</span>
				</div>
			</div>
		</div>
     ...
     首页 上一页
										...
										<a
											href="${ pageContext.request.contextPath }/GetAllJobServlet?pageNum=1">[首页]</a>
										<a
											href="${ pageContext.request.contextPath }/GetAllJobServlet?pageNum=${ pageNum-1 }">[上一页]</a>
      ...
      [${pageNum1 }]
										...
										<c:if test="${pageNum1!=pageNum }">
											<a
												href="${ pageContext.request.contextPath }/GetAllJobServlet?pageNum=${ pageNum1 }">[${ pageNum1 }]</a>
										</c:if>
									</c:forEach> <c:if test="${pageNum==page }">
      下一页   末页
   </c:if> <c:if test="${pageNum<page }">
										<a
											href="${ pageContext.request.contextPath }/GetAllJobServlet?pageNum=${ pageNum+1 }">下一页</a>
										<a
											href="${ pageContext.request.contextPath }/GetAllJobServlet?pageNum=${ page}">末页</a>
									</c:if></td>
								</tr>
							</tbody>
						</table>
